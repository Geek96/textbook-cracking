...
### Practice Problems (optional, on request only — math model)

Not part of default Ingest; skip unless the user explicitly asks for practice problems for one or more chapters. Read the selected model's own "Practice problems" section (currently defined for math — see `content-models/math.md`) for admission rules, scope-grounding against a CanvasManager-synced course syllabus when one is present, difficulty calibration, and the bilingual format. These are Agent-original problems, never the textbook's own exercises — see `references/course-manager-integration.md`'s "Exercise handoff" for why that distinction matters and stays with CourseManager.

### Week Practice (optional, on request only — CS model)

Not part of default Ingest; skip unless explicitly asked, and only after the relevant Week page(s) already exist. Read the CS content model's own "Week practice" section (`content-models/cs.md`) for the two sub-types (`选择题练习/`+`选择题答案/` MCQs with a separated answer key, and `编程练习/` link-only curated external problems), sourcing discipline for the "modeled on the professor's slides" subset, and volume/workload agreement with the user. Same exercise-handoff boundary as the math model above applies here too.
