...
Week-scoped practice content (`选择题练习/`, `选择题答案/`, `编程练习/`) is
generated by the separate `course-manager` skill, not this one — it reads
`周次/` read-only for that — see `../course-manager-integration.md`.
...
Week-scoped practice content (MCQs + answer key, curated external coding
problems) for a week that already has a Week page is generated by the
separate `course-manager` skill (not this one) — see
`../course-manager-integration.md` for the sub-types, sourcing discipline,
and format rules, ported from here as of v0.2.0.
...
(Week practice pages, when used, are course-manager's output, not this
skill's — see `../course-manager-integration.md`.)
