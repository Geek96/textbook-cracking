...
`练习题/` pages (the full-parallel-pair bilingual pattern referenced above)
are generated by the separate `course-manager` skill, not this one — see
`course-manager-integration.md` for that bilingual rule and everything
else about practice-problem/week-practice generation, ported there as of
v0.2.0.
